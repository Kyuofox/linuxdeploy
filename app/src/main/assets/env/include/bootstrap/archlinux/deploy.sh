...
do_install()
{
    ...
    *) return 1 ;;
    ...
    local core_files=$(wget -q -O - "${repo_url}/core.db.tar.gz" | tar xOz | grep '.pkg.tar.xz$' | grep -v -e '^linux-' -e '^grub-' -e '^efibootmgr-' -e '^openssh-' -e 'ca-certificates-cacert' -e 'iptables-nft' -e 'openssl-cryptodev' -e 'systemd-resolvconf' -e 'debug' -e 'qgpgme' -e 'libgccjit' -e 'llvm-libs' | sort)
    is_ok "fail" "done" || return 1
